"""Create one machine-learning row for each customer."""
        ...
        training_rows.append(
            {
                ...
                "days_since_last_event": (now - row["last_event_at"]).days,
                "label_high_value": int(total_value > 200),
            }
        )
...
def export_training_data(output_path="training_data.csv"):
    rows = prepare_training_rows(get_customer_aggregates())
    write_csv(rows, output_path)
    print(f"Training data exported to {output_path}")
    print(f"Customers exported: {len(rows)}")
    return rows


if __name__ == "__main__":
    export_training_data(os.getenv("OUTPUT_PATH", "training_data.csv"))
